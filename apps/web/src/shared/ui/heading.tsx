...
interface HeadingProps extends React.ComponentProps<HeadingTag>, PropsWithSlotReplacer {
  as: HeadingTag;
}

export function Heading({ children, className, as: As, asChild = false, ...props }: HeadingProps) {
  const Component = asChild ? Slot : As;

  return (
    <Component className={cn(HeadingVariants({ as: As }), className)} {...props}>
      {children}
    </Component>
  );
}
