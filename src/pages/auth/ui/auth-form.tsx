...
export function AuthForm() {
  const { stage = AuthFormStageMap.EMAIL } = routeApi.useSearch();
  ...
  const canGoToCodeStage = isEmailStage && isCodeStageVisitedRef.current;

  const goToCodeStage = useCallback(() => {
    navigate({ search: { stage: AuthFormStageMap.CODE } });
    isCodeStageVisitedRef.current = true;
  }, [navigate]);

  const goToEmailStage = useCallback(() => {
    navigate({ search: { stage: AuthFormStageMap.EMAIL } });
  }, [navigate]);
  ...
  const formRender = useMemo(() => {
    if (isCodeStage) {
      return <AuthCodeForm onSuccess={goToEmailStage} />;
    }

    return <AuthEmailForm onSuccess={goToCodeStage} />;
